...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Click the Play Button to Start", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(noseX != "" && Game_stats == "start" && gameConfig.status==="start") {
    document.getElementById("stats").innerHTML = "Game is Loaded";
    world_start.play();
